...
# SỬA ENCODING: ép stdout/stderr sang UTF-8 để print emoji/tiếng Việt KHÔNG sập
# khi bị Electron spawn (mặc định Windows là cp1252, không encode nổi ⏳/✅ -> crash).
# Bọc if vì khi đóng gói không console, sys.stdout có thể là None.
if sys.stdout:
...
# ============================================================
# SỬA 1a: khi đóng gói .exe, models nằm CẠNH file exe (không phải thư mục tạm)
...
print("⏳ Đang nạp model Dịch M2M-100 418MB (CTranslate2 INT8)...")
...
# Sau lần chạy đầu, thư mục m2m100_418M_int8 sẽ có thêm file tokenizer -> ship kèm app là xong.
try:
            ...
            while not vad.empty():
                ...
                for sentence in split_sentences(text_src, src_lang):
                    ...
                    await websocket.send_text(
                        json.dumps({"src": sentence, "dst": text_dst}, ensure_ascii=False)
                    )
...
if __name__ == "__main__":
    # SỬA 1b: truyền thẳng object app (không dùng chuỗi "main:app") -> hợp PyInstaller + hết nạp model đôi
